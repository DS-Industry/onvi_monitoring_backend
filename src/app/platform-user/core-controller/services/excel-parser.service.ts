...
export class FileParserService {
  async parseCardImportFile(file: Express.Multer.File): Promise<CardImportItemDto[]> {
    try {
  ...
  private async parseCSVFile(file: Express.Multer.File): Promise<CardImportItemDto[]> {
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (columns.length >= 3) {
          ...
          if (card.devNumber && card.uniqueNumber && card.tierId > 0) {
            ...
            console.warn('Skipping invalid CSV row:', { line, card, rowIndex: i });
          }
        } else {
          console.warn('Skipping CSV row with insufficient columns:', { line, rowIndex: i });
        }
      }
  ...
  private async parseExcelFile(file: Express.Multer.File): Promise<CardImportItemDto[]> {
    try {
      ...
      for (let i = 1; i < jsonData.length; i++) {
        ...
        if (row.length >= 3) {
          ...
          if (card.devNumber && card.uniqueNumber && card.tierId > 0) {
            ...
            console.warn('Skipping invalid Excel row:', { row, card, rowIndex: i });
          }
        } else {
          console.warn('Skipping Excel row with insufficient columns:', { row, rowIndex: i });
        }
      }
