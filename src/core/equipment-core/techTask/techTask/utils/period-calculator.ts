...
export class PeriodCalculator {
    ...
    switch (periodType) {
      ...
      case PeriodType.CUSTOM:
        if (!customPeriodDays || customPeriodDays <= 0) {
          throw new Error(
            'Custom period days must be a positive number when using CUSTOM period type',
          );
        }
  ...
  static validatePeriodConfig(
    ...
    if (periodType === PeriodType.CUSTOM) {
      ...
      throw new Error(
        `Custom period days should not be provided for ${periodType} period type`,
      );
    }
